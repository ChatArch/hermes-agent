...
class GatewaySessionCommandsMixin:
    ...
    async def _handle_compress_command(self, event: MessageEvent, *, local_fallback_only: bool = False) -> str:
        ...
        if not getattr(getattr(self, "config", None), "multiplex_profiles", False):
            return await self._handle_compress_command_inner(event, local_fallback_only=local_fallback_only)
    ...
    async def _handle_compress_command_inner(self, event: MessageEvent, *, local_fallback_only: bool = False) -> str:
        ...
        try:
            return await self._run_manual_compression(source, session_entry, history, partial,
                                                      keep_last, focus_topic, local_fallback_only=local_fallback_only)
        except Exception as e:
            logger.warning("Manual compress failed: %s", e)
            return t("gateway.compress.failed", error=e)

    async def _run_manual_compression(self, source, session_entry, history: list, partial: bool,
                                      keep_last, focus_topic, *, local_fallback_only: bool = False) -> str:
        ...
        if str(runtime_kwargs.get("api_mode") or "").lower() == "codex_app_server":
            if local_fallback_only:
                return t("gateway.compress.failed", error="Local compression is unsupported for native Codex app-server sessions")
        ...
        lines = _manual_compression_reply_lines(summary, compressor, focus_topic)
        if local_fallback_only:
