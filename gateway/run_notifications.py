...
class GatewayNotificationsMixin:
    ...
    async def _deliver_media_from_response(
        ...
        *, streamed_message_id=None, session_id=None, fallback_session_id=None, session_key=None,
    ) -> None:
        ...
        if session_id and session_key and "MEDIA:" in response:
            materialized = await self._materialize_media_for_delivery(
                response, session_id=session_id, fallback_session_id=fallback_session_id, session_key=session_key)
        ...
        with _log_suppressed(logging.WARNING, "Post-stream media extraction failed: %s"):
            ...
            if event.source.platform == Platform.FEISHU and len(image_paths) == 1 and not non_image_media:
                result = await adapter.send_image_file(
                    chat_id=chat_id, image_path=image_paths[0], caption=cleaned.strip() or None,
                    reply_to=self._reply_anchor_for_event(event), metadata=_thread_meta,
                )
                if result.success and streamed_message_id and streamed_message_id != "__no_edit__":
                    await adapter.delete_message(chat_id, streamed_message_id)
                return
            if image_paths:
                try:
    ...
    async def _deliver_queued_first_response(
        ...
        await self._deliver_media_from_response(
            ...
            streamed_message_id=getattr(stream_consumer, "message_id", None),
        )
