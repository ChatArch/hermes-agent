...
def _mask_media_scan_text(text: str) -> str:
    ...
    return _mask_json_resource_uri_media(A._mask_json_string_media(A._mask_protected_spans(text)))
...
def _real_media_tag_spans(masked: str) -> list:
    ...
    spans.extend(match.span() for match in MEDIA_RESOURCE_URI_RE.finditer(masked))
...
class BasePlatformAdapter(ABC):
    ...
    async def _dispatch_inline_reply(self, event: MessageEvent, *, log_cmd: Optional[str] = None) -> None:
        ...
        if isinstance(response, CardReply):
            await self.send_card_reply(
                event.source.chat_id, response, reply_to=_reply_anchor_for_event(event),
    ...
    async def _send_inline_response_media(self, event, extracted, metadata):
        if (self.platform != Platform.FEISHU or len(extracted.media_files) != 1
                or extracted.images or extracted.local_files or extracted.force_document_attachments):
            return None
        path, is_voice = extracted.media_files[0]
        if is_voice or Path(path).suffix.lower() not in {".jpg", ".jpeg", ".png", ".webp", ".gif"}:
            return None
        try:
            return await self.send_image_file(
                chat_id=event.source.chat_id, image_path=path, caption=extracted.text_content or None,
                reply_to=_reply_anchor_for_event(event), metadata=metadata)
        except Exception as exc:
            logger.warning("[%s] Inline image delivery failed: %s", self.name, exc)
            return None
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if not response:
                ...
                inline_result = await self._send_inline_response_media(event, extracted, _final_thread_metadata)
