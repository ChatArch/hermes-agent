...
class GatewayBusySessionMixin:
    ...
    def _gateway_idle_command_handlers(self):
        ...
        handlers = self._command_handler_table(self._IDLE_COMMANDS)
        handlers["compress-local"] = self._handle_compress_local_command
        handlers["cron"] = self._handle_cron_command
        return handlers

    async def _handle_compress_local_command(self, event):
        return await self._handle_compress_command(event, local_fallback_only=True)
    ...
    _BUSY_SPECIAL_HANDLERS: Dict[str, str] = {
        k: f"_busy_{k}_command" for k in ("start", "stop", "new", "interrupt", "queue", "steer", "egress", "goal", "loop")
    }
