...
class TurnRunner:
    ...
    def _resolve_turn_agent(self, turn_route, platform_key, combined_ephemeral, max_iterations, reasoning_config, pr):
        ...
        overrides = resolve_binding_task_overrides(ctx.session_key)
        cache_keys = dict(runner._extract_cache_busting_config(ctx.user_config) or {})
        cache_keys["terminal.section_backend"] = overrides or None
    ...
    def _ssh_grant_notify_sync(self, grant_data):
        from gateway.run import safe_schedule_threadsafe
        ctx = self._ctx
        adapter = ctx._status_adapter
        adapter.pause_typing_for_chat(ctx._status_chat_id)
        send_approval = getattr(adapter, "send_ssh_grant_approval", None)
        if not callable(send_approval):
            raise RuntimeError("SSH grant approval card unavailable for this adapter")
        future = safe_schedule_threadsafe(
            send_approval(chat_id=ctx._status_chat_id, session_key=ctx.session_key or "",
                          alias=str(grant_data.get("alias") or ""), reason=str(grant_data.get("reason") or ""),
                          cwd=grant_data.get("cwd") or None, metadata=ctx._status_thread_metadata),
            ctx._loop_for_step, logger=logger, log_message="SSH approval scheduling failed")
        if future is None:
            raise RuntimeError("SSH approval loop unavailable")
        result = future.result(timeout=15)
        if not getattr(result, "success", False):
            raise RuntimeError(str(getattr(result, "error", None) or "SSH approval send failed"))
    ...
    def _run_conversation_with_approval(self, agent, agent_history, observed_group_context,
        ...
        from tools.ssh_mode_tool import register_gateway_ssh_grant_notify, unregister_gateway_ssh_grant_notify
        from tools.approval_context import reset_current_session_key, set_current_session_key
        ctx = self._ctx
        session_key = ctx.session_key or ""
        token = set_current_session_key(session_key)
        register_gateway_notify(session_key, self._approval_notify_sync)
        register_gateway_ssh_grant_notify(session_key, self._ssh_grant_notify_sync)
        try:
            ...
            unregister_gateway_ssh_grant_notify(session_key)
    ...
    def run_sync(self):
        ...
        backend_overrides = resolve_binding_task_overrides(ctx.session_key)
        if backend_overrides:
            register_task_env_overrides(ctx.session_id, backend_overrides)
        else:
            clear_task_env_overrides(ctx.session_id)
