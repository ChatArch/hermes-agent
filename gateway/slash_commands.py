...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_compress_command(
        ...
        *,
        ...
        """Profile-scoped wrapper for /compress and /compress-local.

        ``/compress`` uses the normal LLM-first summarizer. ``/compress-local``
        skips the summarizer and writes the deterministic local fallback through
        the same guarded rotation/in-place archive machinery.

        Multiplexed gateways resolve credentials through the fail-closed
        per-profile secret scope. Slash-command dispatch does not install the
        agent turn's scope, so wrap the full handler here. The local rescue also
        uses this scope for session/runtime resolution even though it does not
        call an auxiliary model.
        """
        if not getattr(getattr(self, "config", None), "multiplex_profiles", False):
            return await self._handle_compress_command_inner(
                event, local_fallback_only=local_fallback_only
            )
    ...
    async def _handle_compress_command_inner(
        ...
        *,
        ...
        """Handle manual conversation compression.
