...
class StreamTransportMixin:
    ...
    async def _try_fresh_final(self, text: str, *, is_turn_final: bool = True) -> bool:
        ...
        replacement = self._fresh_final_preview_replacement_text(text)
        if replacement:
            for message_id in stale_ids:
                if message_id == new_message_id:
                    continue
                try:
                    await self._edit_message(message_id=message_id, content=replacement, finalize=False)
                except Exception as exc:
                    logger.debug("Fresh-final preview replacement failed: %s", exc)
        else:
