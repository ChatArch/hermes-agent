...
def test_run_conversation_codex_output_cap_continues_instead_of_invalid_retry(monkeypatch):
    """Codex max_output_tokens responses should continue without generic length fallback."""
    agent = _build_agent(monkeypatch)
    responses = [
    ...
    assert any(
        "Partial answer" in (msg.get("content") or "")
        for msg in result["messages"]
    )


def test_run_conversation_codex_empty_output_cap_returns_bounded_error(monkeypatch):
    """Empty max_output_tokens responses should fail after bounded retries."""
    agent = _build_agent(monkeypatch)
    calls = {"count": 0}
