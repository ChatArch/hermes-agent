...
class TestOpenRouterProfile:
    ...
    def test_mandatory_anthropic_verbosity_is_value_agnostic_passthrough(self):
        ...
        clamp or whitelist. ``xhigh``, ``max``, and ``ultra`` are real config
        values; OpenRouter accepts extended efforts for Claude (live-proven in
        #43432), so a forward value must survive
