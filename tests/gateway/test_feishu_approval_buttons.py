...
class TestCardActionCallbackResponse:
    ...
    @pytest.mark.parametrize("group_policy,allowed_users", [("allowlist", set()), ("open", {"ou_allowed"})])
    def test_update_prompt_unauthorized_click_rejected(self, _patch_callback_card_types, group_policy, allowed_users):
        adapter = _make_adapter()
        adapter._loop = MagicMock()
        adapter._loop.is_closed = MagicMock(return_value=False)
        adapter._allowed_group_users = allowed_users
        adapter._group_policy = group_policy
        adapter._default_group_policy = group_policy
        adapter._has_explicit_default_group_policy = True
    ...
    @pytest.mark.parametrize("chat_type,allowed", [("group", False), ("dm", True)])
    def test_prompt_authorization_uses_canonical_session_scope(self, _patch_callback_card_types, chat_type, allowed):
        adapter = _make_adapter()
        adapter._loop = MagicMock()
        ...
        adapter._approval_state[29] = {
            "session_key": f"agent:main:feishu:{chat_type}:oc_chat",
            "message_id": "om_prompt", "chat_id": "oc_chat",
        }
        data = _make_card_action_data({"hermes_action": "approve_once", "approval_id": 29}, chat_id="oc_chat")
        with patch("asyncio.run_coroutine_threadsafe", side_effect=_close_submitted_coro) as submitted:
            response = adapter._on_card_action_trigger(data)
        assert (response.card is not None) is allowed
        assert bool(submitted.call_count) is allowed
