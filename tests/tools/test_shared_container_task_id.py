...
from tools import terminal_tool, terminal_tool_lifecycle
...
def test_get_active_env_reads_shared_container_from_subagent_id():
    ...
    try:
        assert terminal_tool_lifecycle.get_active_env("subagent-7-cafe") is sentinel
        assert terminal_tool_lifecycle.get_active_env(None) is sentinel
        assert terminal_tool_lifecycle.get_active_env("default") is sentinel
    finally:
...
def test_get_active_env_honours_rl_override():
    ...
    try:
        ...
        assert terminal_tool_lifecycle.get_active_env("rl-42") is rl_env
    finally:
...
def test_session_key_scopes_to_its_own_slot(monkeypatch):
    ...
    assert terminal_tool._resolve_container_task_id(None) == (
        terminal_tool._session_environment_key_from_raw("sess-A")
    )
...
def test_distinct_session_keys_get_distinct_slots(monkeypatch):
    ...
    assert a == terminal_tool._session_environment_key_from_raw("sess-A")
    assert b == terminal_tool._session_environment_key_from_raw("sess-B")
    assert a != b
...
def test_session_key_from_contextvar_without_environ(monkeypatch):
    ...
    try:
        expected = terminal_tool._session_environment_key_from_raw("sess-ctx")
        assert (
            terminal_tool._resolve_container_task_id(None) == expected
        )
...
def test_contextvar_session_key_wins_over_environ(monkeypatch):
    ...
    try:
        assert (
            ...
            == terminal_tool._session_environment_key_from_raw("sess-CTX")
        )
    finally:
        clear_session_vars(tokens)
...
def test_nonpersistent_docker_keeps_session_scoping(monkeypatch):
    ...
    try:
        assert terminal_tool._resolve_container_task_id(None) == terminal_tool._session_environment_key_from_raw("sess-A")
        assert terminal_tool._resolve_container_task_id(None) != terminal_tool._session_environment_key_from_raw("sess-B")
    finally:
        clear_session_vars(tokens)
