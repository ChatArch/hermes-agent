...
class TestReadFileToolIntegration(unittest.TestCase):
    ...
    def test_read_policy_runs_before_document_extraction(self):
        """Structured conversion must not bypass the shared read denylist."""
        p = os.path.join(self.tmp, "blocked.docx")
        _write_docx(
            p,
            f'<w:document xmlns:w="{_NS_W}"><w:body>'
            "<w:p><w:r><w:t>secret body</w:t></w:r></w:p>"
            "</w:body></w:document>",
        )
        with (
            mock.patch(
                "tools.file_tools.get_read_block_error",
                return_value="Blocked by shared read policy",
            ),
            mock.patch("tools.read_extract.extract_document_bytes") as extract,
        ):
            res = json.loads(read_file_tool(p))

        self.assertIn("Blocked by shared read policy", res["error"])
