...
class TestSessionIsolationKeying:
    ...
    def test_clearing_child_override_does_not_evict_parent_alias(
        self, monkeypatch
    ):
        """A child owns its isolated override, not the borrowed parent env."""
        _enable_isolation(monkeypatch)
        child = "subagent-override"
        parent = "tui:sess-a"
        parent_env = object()
        child_env = object()
        ...
        terminal_tool.register_task_env_overrides(
            child,
            {"env_type": "ssh", "ssh_host": "example.invalid"},
        )
        with terminal_tool._env_lock:
            terminal_tool._active_environments[parent] = parent_env
            terminal_tool._active_environments[child] = child_env

        try:
            terminal_tool.clear_task_env_overrides(child)
            assert terminal_tool._active_environments.get(parent) is parent_env
            assert child not in terminal_tool._active_environments
        finally:
            with terminal_tool._env_lock:
                terminal_tool._active_environments.pop(parent, None)
                terminal_tool._active_environments.pop(child, None)
