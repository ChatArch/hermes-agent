...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    @unittest.skipIf(sys.platform == "win32", "UDS not available on Windows")
    def test_empty_or_nonoverlapping_tools_do_not_widen_permissions(self):
        """An explicit session policy with no safe overlap grants no tools."""
        code = (
            "import hermes_tools\n"
            "print('terminal_available:', hasattr(hermes_tools, 'terminal'))\n"
        )
        for index, enabled_tools in enumerate(
            ([], ["vision_analyze", "browser_snapshot"])
        ):
            with self.subTest(enabled_tools=enabled_tools):
                result = json.loads(execute_code(
                    code,
                    task_id=f"test-no-safe-tools-{index}",
