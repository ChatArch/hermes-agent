...
class TestByteLayerBinaryDetection:
    ...
    def test_invalid_trailing_start_byte_is_binary(self, file_ops):
        # A final FF is invalid, not an incomplete sampled UTF-8 sequence.
        assert file_ops._is_likely_binary_bytes(b"plain text\xff") is True

    def test_incomplete_tail_at_file_eof_is_binary(self, file_ops):
        # The boundary exception applies only when more file bytes exist.
        assert file_ops._is_likely_binary_bytes(
            b"plain text\xe4",
