...
def build_environment_hints() -> str:
    ...
    backend = _current_task_backend((_tenv_read("TERMINAL_ENV") or "local").strip().lower())
    ...
    if backend == "ssh":
        ...
        if ssh_alias:
            hints.append(
                ...
                "created on this target through the messaging gateway, use "
                f"`MEDIA:ssh://{ssh_alias}/absolute/remote/path.ext` in the "
                "final response (percent-encode spaces). Do not label a "
                "remote path as `file://`; `file://` is reserved for files "
                "on the Hermes gateway machine."
            )
