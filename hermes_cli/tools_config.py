...
def _enabled_plugin_toolsets(config: dict, platform: str, toolset_names: List[str], plugin_ts_keys: Set[str]) -> Set[str]:
    ...
    explicitly_configured = isinstance((config.get("platform_toolsets") or {}).get(platform), list)
    return {
        pts for pts in plugin_ts_keys
        if pts in toolset_names or (
            not explicitly_configured and pts not in _DEFAULT_OFF_TOOLSETS and pts not in known_for_platform
        )
    }
