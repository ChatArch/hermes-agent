...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # Bundled native tool plugins auto-load too: when Hermes ships a
            # first-party tool surface, models should be able to call it
            # naturally without a separate Skill/config step. Bundled platform
            # plugins are registered lazily below so CLI startup does not import
            # every platform SDK.
            if manifest.source == "bundled" and manifest.kind in {"backend", "tool"}:
