...
def launchd_restart():
    ...
    try:
        ...
        _write_planned_restart_notification_marker(source="launchd", via_cli=True, hard_restart=True)
...
def _running_under_gateway_supervisor() -> bool:
    """Detect gateway supervision without treating non-macOS XPC markers as launchd."""
...
def _cmd_restart(args):
    ...
    if os.getenv("_HERMES_GATEWAY") == "1":
        if not getattr(args, "all", False) and _request_current_gateway_self_restart():
            return
        print_error("Refusing ambiguous gateway-hosted restart; use /restart in chat.")
        sys.exit(1)
    system = getattr(args, "system", False)
