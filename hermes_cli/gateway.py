...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        # First distinguish the actual supervised gateway process from a
        # gateway-hosted descendant. The gateway itself must never signal or
        # service-restart itself from this CLI path.
        from tools.process_registry import _is_supervised_gateway_process
        ...
        # A gateway-hosted child may request the current gateway's safe,
        # drain-aware SIGUSR1 restart path. Refuse ambiguous self-targets.
