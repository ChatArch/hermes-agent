...
def _rpc_poll_loop(env, rpc_dir: str, task_id: str, tool_call_log: list, tool_call_counter: list,
    ...
    while not stop_event.is_set():
        try:
            ls_result = _execute_remote_internal(env, f"ls -1 {quoted_rpc_dir}/req_* 2>/dev/null || true", cwd="/", timeout=10)
            ...
            for req_file in req_files:
                ...
                read_result = _execute_remote_internal(env, f"cat {quoted_req_file}", cwd="/", timeout=10)
                try:
