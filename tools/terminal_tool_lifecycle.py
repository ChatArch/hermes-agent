...
def ensure_task_env(task_id: Optional[str] = None):
    ...
    from tools.terminal_tool import (
        _active_environments, _creation_locks, _creation_locks_lock, _env_lock,
        _get_env_config, _last_activity, _resolve_container_task_id, apply_task_env_overrides,
...
def cleanup_vm(task_id: str, *, force_remove: bool = False):
    ...
    *force_remove* is forwarded to backends that accept it (currently only
    ...
    """
    from tools.terminal_tool import _session_environment_key_from_raw

    cleaned = set()
    for key in {task_id, _session_environment_key_from_raw(task_id)}:
        env = _unregister_env(key)
        _clear_file_ops_cache(key)
        if env is None or id(env) in cleaned:
            continue
        cleaned.add(id(env))
        _teardown_env(
