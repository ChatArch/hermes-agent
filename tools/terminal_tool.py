...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "ssh_identities_only": _tenv("TERMINAL_SSH_IDENTITIES_ONLY", "auto").lower() in {"true", "1", "yes", "on", "auto"},
        "ssh_known_hosts": _tenv("TERMINAL_SSH_KNOWN_HOSTS", ""),
        "ssh_host_key_policy": _tenv("TERMINAL_SSH_HOST_KEY_POLICY", "accept-new"),
...
def apply_task_env_overrides(config: Dict[str, Any], overrides: Dict[str, Any] | None) -> Dict[str, Any]:
    ...
    if ov.get("cwd"):
        ...
        if _is_container_backend(merged.get("env_type")) and _is_unusable_container_cwd(override_cwd):
            logger.info(
