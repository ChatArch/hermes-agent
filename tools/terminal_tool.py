    ...
    *,
    ...
    if environment_keys is None:
        ...
        with _container_alias_lock:
            is_aliased_child = task_id in _container_aliases
        resolved = _resolve_container_task_id(task_id)
        # An aliased child borrows its parent's environment; evicting the
        # child's own override/cache must never tear down that parent. Likewise
        # an arbitrary legacy task that collapses to the shared "default" slot
        # does not own that shared environment.
        if not is_aliased_child and (
            resolved != "default" or task_id == "default"
        ):
            keys.add(resolved)
    else:
        keys = {key for key in environment_keys if key}
    evicted = []
    with _env_lock:
        for key in keys:
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    CWD-only overrides are not hard-isolation signals; only backend/image
    overrides force raw task-id isolation. Non-persistent Docker sessions keep
    official per-task alias isolation. Other gateway-backed environments use a
    deterministic backend-safe key derived from the active session so chats and
    threads cannot share cwd, env snapshots, or SSH connections.
    """
    ...
    *force_remove* (default False) is forwarded to backends that accept it
    ...
    with _env_lock:
        for key in keys:
            env = _active_environments.pop(key, None)
            _last_activity.pop(key, None)
            if env is not None and all(env is not known for known in envs):
                envs.append(env)

    # Clean up per-task creation locks
    with _creation_locks_lock:
        for key in keys:
            _creation_locks.pop(key, None)

    # Invalidate stale file_ops cache entries
    try:
        from tools.file_tools import clear_file_ops_cache
        for key in keys:
            clear_file_ops_cache(key)
