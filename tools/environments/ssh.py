...
def _ensure_ssh_available(*, require_scp: bool = False) -> None:
...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
                 ...
                 sync_hermes_files: bool = False, probe_only: bool = False):
        ...
        _ensure_ssh_available(require_scp=self._sync_hermes_files)
    ...
    def _make_control_dir(cls) -> Path:
        ...
        if not _SSH_MULTIPLEX:
            # Windows OpenSSH does not use this path as a Unix-domain control
            # socket. Its POSIX ownership/mode bits are not meaningful there.
            return control_dir
    ...
    def _target_flags(self, port_flag: str) -> list:
        ...
        flags.extend(["-o", f"UserKnownHostsFile={self.known_hosts_path}",
                      "-o", f"StrictHostKeyChecking={self.host_key_policy}"])
        if self.key_path and self.identities_only:
            flags.extend(["-o", "IdentitiesOnly=yes"])
    ...
    def _build_ssh_command(self, extra_args: list | None = None, send_env: Iterable[str] = ()) -> list:
        ...
        cmd.extend(["-o", "BatchMode=yes", "-o", "ConnectTimeout=10"])
