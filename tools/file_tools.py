...
from tools.file_tools_paths import (
    _file_tool_paths_for_task, _is_remote_backend_task, _remote_display_path_for_task,
...
def _create_terminal_env_for_file_ops(raw_task_id: str, task_id: str):
    ...
    config = apply_task_env_overrides(_get_env_config(), resolve_task_overrides(raw_task_id))
...
def read_file_tool(path: str, offset: int = 1, limit: int = DEFAULT_READ_LIMIT, task_id: str = "default") -> str:
    ...
    try:
        ...
        _resolved = (Path(_remote_display_path_for_task(path, task_id)) if is_remote_backend
                     else _resolve_path_for_task(path, task_id))
...
def _edit_warnings(paths: list[str], path_to_resolved: dict, task_id: str) -> list[str]:
    ...
    for p in paths:
        ...
        if not w and r and not _is_remote_backend_task(task_id):
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    try:
        ...
        with ExitStack() as _lock:
            ...
            result_dict = _get_file_ops(task_id).write_file(_file_tool_paths_for_task(path, task_id)[1], content).to_dict()
            if warnings:
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    try:
        ...
        _path_to_shell = {path: _file_tool_paths_for_task(path, task_id)[1] for path in _paths_to_check}
        with ExitStack() as _locks:
            ...
            if mode == "replace":
                ...
                result = file_ops.patch_v4a(_rewrite_v4a_patch_paths_for_host(patch, _path_to_shell, file_ops))
            else:
