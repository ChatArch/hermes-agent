...
def _resolve_sandbox_tools(
    ...
    """Return the sandbox allow-list for a session.

    ``None`` is the legacy "no session list supplied" default and keeps all
    sandbox-safe tools. An explicit empty or disjoint list grants no tools;
    silently widening that case to every allowed tool defeats session policy.
    """
