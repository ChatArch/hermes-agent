...
import json
import logging
from tools.code_execution_env import _execute_remote_internal
import os
import re
import secrets
import shlex
...
def _get_or_create_env(task_id: str):
    ...
    from tools.terminal_tool import (
        ...
        resolve_task_overrides, apply_task_env_overrides,
    )
    ...
    env = _cached()
    if env is not None:
        return env, apply_task_env_overrides(_get_env_config(), resolve_task_overrides(task_id))["env_type"]
...
def _ship_file_to_remote(env, remote_path: str, content: str) -> None:
    ...
    _execute_remote_internal(env, f"echo '{encoded}' | base64 -d > {shlex.quote(remote_path)}", cwd="/", timeout=30)
...
def _sandbox_tools_for(enabled_tools: Optional[List[str]]) -> frozenset:
    """Resolve the session's sandbox tool grant without widening an empty grant."""
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    try:
        _execute_remote_internal(env, f"mkdir -p {quoted_rpc_dir}", cwd="/", timeout=10)
        ...
        script_result = _execute_remote_internal(env, f"cd {quoted_sandbox_dir} && {env_prefix} python3 script.py",
                                    timeout=timeout)
        ...
        try:
            _execute_remote_internal(env, f"rm -rf {quoted_sandbox_dir}", cwd="/", timeout=15)
        except Exception:
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        py_check = _execute_remote_internal(env, "command -v python3 >/dev/null 2>&1 && echo OK", cwd="/", timeout=15)
