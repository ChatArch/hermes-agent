...
class ShellFileOperations(FileOperations):
    ...
    def read_file(self, path: str, offset: int = 1, limit: int = 2000) -> ReadResult:
        ...
        if sample_bytes is not None:
            ...
            is_binary = ext_binary or self._is_likely_binary_bytes(
                sample_bytes,
                allow_truncated_tail=file_size > len(sample_bytes),
            )
        else:
            ...
            if "\ufffd" in sample_output[:1000]:
                raw_utf8_status = self._raw_utf8_sample_status(
                    path,
                    sample_bytes=1000,
                    file_size=file_size,
                )
            is_binary = self._is_likely_binary(
                path,
                sample_output,
                raw_utf8_status=raw_utf8_status,
            )
