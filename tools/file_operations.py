...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _is_likely_binary_bytes(
        ...
        *,
        ...
        try:
            ...
            # Only an actually incomplete UTF-8 sequence at a sampled boundary
            # is text. Invalid trailing bytes such as FF are binary even though
            # their error offset also falls in the last three bytes.
            if (
    ...
    def _is_likely_binary(
        ...
        *,
        ...
        if content_sample:
            ...
            # overwrite the original bytes with mojibake.
            #
            # A fixed-size byte sample can also end in the middle of a valid
            # UTF-8 character, especially for Chinese/emoji Markdown over SSH;
            # that *also* arrives as U+FFFD even though the file is text. When
            # the caller has a raw-byte probe proving the sample is valid UTF-8
            # apart from a partial trailing sequence, do not classify it as
            # binary merely because the lossy terminal text contains U+FFFD.
            if "\ufffd" in content_sample[:1000] and raw_utf8_status != "text":
                return True
    ...
    def _detect_binary(self, path: str, file_size: Optional[int] = None) -> tuple[bool, Optional[bytes]]:
        ...
        if sample_bytes is not None:
            ...
            return ext_binary or self._is_likely_binary_bytes(
                sample_bytes, allow_truncated_tail=file_size is None or file_size > len(sample_bytes),
            ), sample_bytes
        sample_output = _strip_terminal_fence_leaks(self._head(path, 1000).stdout)
        raw_utf8_status = self._raw_utf8_sample_status(path, file_size=file_size) if "\ufffd" in sample_output[:1000] else None
        return self._is_likely_binary(path, sample_output, raw_utf8_status=raw_utf8_status), None
    ...
    def read_file(self, path: str, offset: int = 1, limit: int = 2000) -> ReadResult:
        ...
        if sample_bytes is not None:
            is_binary = self._is_likely_binary_bytes(sample_bytes, allow_truncated_tail=file_size > len(sample_bytes))
        else:
            logger.debug(
            ...
            sample_output = _strip_terminal_fence_leaks(self._head(path, 1000).stdout)
            raw_utf8_status = self._raw_utf8_sample_status(path, file_size=file_size) if "\ufffd" in sample_output[:1000] else None
            is_binary = self._is_likely_binary(path, sample_output, raw_utf8_status=raw_utf8_status)
        if is_binary:
    ...
    def _read_file_native(self, path: str, offset: int, limit: int) -> ReadResult:
        ...
        try:
            with open(full, "rb") as fh:
                sample = fh.read(1000)
                ext_binary = os.path.splitext(path)[1].lower() in BINARY_EXTENSIONS
                if ext_binary or self._is_likely_binary_bytes(sample, allow_truncated_tail=file_size > len(sample)):
    ...
    def _read_file_sequential(self, path: str, offset: int, limit: int) -> ReadResult:
        ...
        if self._is_image(path):  # never inlined — redirect to the vision tool
            return self._image_redirect_result(file_size)
        is_binary, sample_bytes = self._detect_binary(path, file_size)
        if is_binary:
            return self._read_binary_file(path, offset, limit, file_size, sample_bytes)
