...
_LARK_SDK_IMPORTS = (
    ...
    ("lark_oapi.api.im.v1", (
        "DeleteMessageRequest", "CreateFileRequest", "CreateFileRequestBody", "CreateImageRequest", "CreateImageRequestBody",
...
from gateway.platforms.base import (
    ...
    _mark_notify_metadata, _reply_anchor_for_event, _thread_metadata_for_source,
)
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _handle_registered_card_action(
        ...
        *,
        ...
        if not self._is_interactive_operator_authorized(sender_id, chat_id, chat_type=callback_chat_type or "group"):
    ...
    def _is_interactive_operator_authorized(self, sender_id: Any, chat_id: str, *, chat_type: str = "") -> bool:
        ...
        if self._allowed_group_users and not (sender_ids & self._allowed_group_users):
            return False
        rule = self._group_rules.get(chat_id)
        if rule or self._has_explicit_default_group_policy or self._default_group_policy in {"disabled", "admin_only"}:
            return self._allow_group_message(sender_id, chat_id, is_bot=False)
        allowed_ids = self._admins | self._allowed_group_users
        return not allowed_ids or bool(sender_ids & allowed_ids)

    def _is_prompt_operator_authorized(self, sender_id: Any, state: Dict[str, str], *, default_chat_type: str = "") -> bool:
        sender_ids = {getattr(sender_id, "open_id", ""), getattr(sender_id, "user_id", "")} - {"", None}
        allowed_ids = self._admins | self._allowed_group_users
        if allowed_ids and "*" not in allowed_ids and not sender_ids & allowed_ids:
            return False
        parts = str(state.get("session_key", "")).split(":", 4)
        chat_type = parts[3] if len(parts) >= 4 and parts[2] == "feishu" else default_chat_type
        return self._is_interactive_operator_authorized(sender_id, state.get("chat_id", ""), chat_type=chat_type)
    ...
    def _handle_ssh_grant_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        user_id = str(getattr(operator, "user_id", "") or "")
        sender_id = SimpleNamespace(open_id=open_id, user_id=user_id)
        if not self._is_prompt_operator_authorized(sender_id, state, default_chat_type="group"):
    ...
    def _validate_card_action(
        ...
        open_id = str(getattr(operator, "open_id", "") or "")
        user_id = str(getattr(operator, "user_id", "") or "")
        if not self._is_prompt_operator_authorized(
                SimpleNamespace(open_id=open_id, user_id=user_id), state):
        ...
        if callback_chat_id and expected_chat_id and callback_chat_id != expected_chat_id:
            ...
            return None
        return open_id, callback_chat_id, self._get_cached_sender_name(open_id) or open_id, user_id
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if checked is None:
            return self._card_response()
        open_id, chat_id, user_name, user_id = checked
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if checked is None:
            return self._card_response()
        open_id, chat_id, user_name, user_id = checked
        coro = self._resolve_update_prompt(prompt_id, answer, user_name, open_id=open_id,
    ...
    async def _resolve_ssh_grant(
        ...
        *,
        ...
        if not state:
            logger.debug("[Feishu] SSH grant %s already resolved or unknown", grant_id)
            return
        if not self._is_prompt_operator_authorized(SimpleNamespace(open_id=open_id, user_id=user_id), state, default_chat_type="group"):
    ...
    async def _resolve_update_prompt(
        ...
        *,
        ...
        state = self._pop_validated_prompt_state(
            ...
            operator_repr=open_id, user_id=user_id,
        )
        if not state:
            return
        try:
    ...
    async def _send_raw_message(
        ...
        # Feishu create-message does not accept thread_id as receive_id_type.
        # Cron deliveries can have a target thread_id but no reply anchor; in
        # that case send to the parent chat instead of failing validation.
        _thread_id = (metadata or {}).get("thread_id")
        if _thread_id:
            logger.warning(
                "[Feishu] Thread metadata for chat %s has no reply anchor; "
                "sending to chat_id instead of invalid thread_id create",
                chat_id,
            )
